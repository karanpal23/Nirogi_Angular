...
@Component({
  selector: 'app-fourth-catog-preview',
  templateUrl: './fourth-catog-preview.component.html',
  styleUrls: ['./fourth-catog-preview.component.css']
})
export class FourthCatogPreviewComponent implements OnInit {
  ...
  @ViewChild('printViewContainer') printViewContainer: ElementRef;
    
    expandAllSections() {
      const accordionItems = this.printViewContainer.nativeElement.querySelectorAll('.accordion-item'); // Select all accordion items
  
      accordionItems.forEach((accordionItem: HTMLElement) => {
    ...
    generatePDF2() {
      const content = this.printViewContainer.nativeElement;
  
      // Get all the accordion items and store their current state
      const accordionItems: NodeListOf<HTMLElement> = content.querySelectorAll('.accordion-item');
      const accordionStates: boolean[] = Array.from(accordionItems).map(item => {
        const isExpanded = item.querySelector('.accordion-collapse').classList.contains('show');
        return isExpanded;
      });
  
      // Expand all sections for generating the PDF
      accordionItems.forEach((accordionItem: HTMLElement) => {
        const collapseElement = accordionItem.querySelector('.accordion-collapse');
        collapseElement.classList.add('show');
      });
  
      // Generate the PDF from the expanded content
      html2canvas(content).then(canvas => {
        ...
        pdf.save('printable_view.pdf');
  
        // Restore the original state of accordion items
        accordionItems.forEach((accordionItem: HTMLElement, index) => {
          const collapseElement = accordionItem.querySelector('.accordion-collapse');

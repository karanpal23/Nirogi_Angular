...
@Component({
  selector: 'app-ref-fourth-category',
  templateUrl: './ref-fourth-category.component.html',
  styleUrls: ['./ref-fourth-category.component.css']
})
export class RefFourthCategoryComponent implements OnInit {
  @ViewChild('first-body') formContent: ElementRef;
  successfull: string;
  obj:ReferenceId;
  msg: string;
  memberId: any;
...
printButton=false;
  onSave(){
    const hBinputField = document.getElementById("HbValue") as HTMLInputElement;
    const hBinputValue = hBinputField.value;

    const rbsinputField = document.getElementById("rbs") as HTMLInputElement;
    const rbsinputValue = rbsinputField.value;

    const relevInvesinputField = document.getElementById("releventInvestigation") as HTMLInputElement;
    const relevInvesinputValue = relevInvesinputField.value;
